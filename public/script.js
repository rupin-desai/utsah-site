...
(function() {
  ...
  if (isMobile) {
    ...
    // Only index.html ships the fly-in intro overlay. Every other page
    // (about/contact/events/socials/team) has no #mobileIntro, so just
    // reveal the nav at its normal CSS position instead of bailing out
    // and leaving the hamburger stuck at opacity:0 forever.
    if (!overlay || !introImg) {
      ...
      if (upcomingBarEl) upcomingBarEl.classList.add('visible');
      return;
    }
    ...
    function runIntro() {
      ...
      requestAnimationFrame(() => {
        ...
        // Phase 3 (2800ms) — logo fades out. The hamburger keeps its normal
        // CSS-defined position (top: safe-area + 14px, centered) instead of
        // being snapped to a JS-measured rect, which drifted mid-screen
        // whenever the fly-up transition hadn't finished settling yet.
        setTimeout(() => {
          introImg.style.transition = 'opacity 0.45s ease';
          introImg.style.opacity    = '0';
        }, 2800);

        // Phase 4 (3050ms) — hamburger fades in at its normal nav position
        setTimeout(() => {
          if (menu) {
  ...
  // HTML defaults to class="nav__logo-img done" as a no-JS fallback; strip
  // it before animating so 'done' can't tie with center/to-top at equal
  // CSS specificity and silently win (it sits later in the stylesheet).
...
(function() {
  ...
  // here made the desktop scroll-jack code run against a section CSS
  // had already shrunk to 50vh, so the photo card — sized for a 200vh
  // scroll range that no longer existed — ended up overlapping the
  // reviews section below it.
  const isMobileGallery = window.matchMedia('(max-width: 430px)').matches;
...
(function() {
  ...
  var scatterTicking = false;
  window.addEventListener('scroll', function() {
    if (scatterTicking) return;
    scatterTicking = true;
    requestAnimationFrame(function() {
      update();
      scatterTicking = false;
    });
  }, { passive: true });
  update();
})();
...
// ===== EDIT MODE =====
(function() {
  fetch('/edit-check').then(r => r.json()).then(data => {
    if (data && data.allowed) initEditMode();
  }).catch(() => {});
  ...
  function initEditMode() {
    ...
    function getEditPassword(forcePrompt) {
      let pw = sessionStorage.getItem('utsahEditPw');
      if (!pw || forcePrompt) {
        pw = window.prompt('Enter edit password:');
        if (pw) sessionStorage.setItem('utsahEditPw', pw);
      }
      return pw;
    }
    ...
    async function doSave(retrying) {
      ...
      const pw = getEditPassword(!!retrying);
      if (!pw) {
        s.style.display = 'inline-block'; s.textContent = 'Cancelled'; s.style.color = '#f44336';
      ...
      try {
        ...
        const r = await fetch('/save', {
          ...
          body: JSON.stringify({ file: f, content: html })
        });
        if (r.status === 401) {
          sessionStorage.removeItem('utsahEditPw');
          if (!retrying) return doSave(true);
          s.textContent = '✗ Wrong password'; s.style.color = '#f44336';
        } else if (r.ok) {
          s.textContent = '✓ Saved!'; s.style.color = '#4caf50';
        } else {
          s.textContent = '✗ Error'; s.style.color = '#f44336';
        }
